...
from api.serializers.general_serializer import SalaSerializer

class SalaListCreateAPIView(generics.ListCreateAPIView):
    serializer_class = SalaSerializer
    
    def not_found(self):
        return Response({"error":"No se encontraron salas"},status=status.HTTP_404_NOT_FOUND)
...
class SalaRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    serializer_class = SalaSerializer

    def not_found(self):
        return Response({"error":"No se encontro una sala con ese ID"})
    ...
    def destroy(self,request,pk=None):
        ...
        if baliza:
            baliza.state = False
            baliza.save()
            return Response({'message':"Sala eliminada correctamente"},status=status.HTTP_200_OK)
        return self.not_found()